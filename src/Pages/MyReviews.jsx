...
const MyReviews = () => {
  ...
  return (
    <div className="max-w-4xl mx-auto p-6 my-6 bg-[#D8EAFC] rounded-lg shadow-lg ">
      <h2 className="text-3xl text-base-300 font-bold mb-6 text-center">My Reviews</h2>
      {error ? (
        ...
        reviews.map((review) => (
          <div
            key={review._id}
            className="border border-gray-300 p-4 rounded-lg shadow-md mb-4"
          >
            <h3 className="text-xl font-semibold text-base-300">
              {review.serviceTitle}
            </h3>
            <p className="mt-2 text-base-300 text-sm md:text-xl">{review.reviewText}</p>
            <Rating
              initialRating={review.rating}
              readonly
              emptySymbol={<span className="text-base-300 text-2xl">☆</span>}
              fullSymbol={<span className="text-yellow-500 text-2xl">★</span>}
            />
            <div className="flex justify-end mt-4 space-x-2">
              <button
                onClick={() => handleEdit(review)}
                className="px-4 py-2 bg-blue-500 text-base-content  font-semibold rounded-lg"
              >
                Edit
              </button>
              <button
                onClick={() => handleDelete(review._id)}
                className="px-4 py-2 bg-red-500 text-base-content font-semibold rounded-lg"
              >
                Delete
              </button>
            </div>
